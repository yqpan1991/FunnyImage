...
import com.edus.apollo.funny.utils.EsShellUtils.CommandResult;
 ...
 * <li>{@link EsPackageUtils#installNormal(Context, String)}</li>
 * <li>{@link EsPackageUtils#installSilent(Context, String)}</li>
 * <li>{@link EsPackageUtils#install(Context, String)}</li>
 * </ul>
 * <ul>
 * <strong>Uninstall package</strong>
 * <li>{@link EsPackageUtils#uninstallNormal(Context, String)}</li>
 * <li>{@link EsPackageUtils#uninstallSilent(Context, String)}</li>
 * <li>{@link EsPackageUtils#uninstall(Context, String)}</li>
 * </ul>
 * <ul>
 * <strong>Is system application</strong>
 * <li>{@link EsPackageUtils#isSystemApplication(Context)}</li>
 * <li>{@link EsPackageUtils#isSystemApplication(Context, String)}</li>
 * <li>{@link EsPackageUtils#isSystemApplication(PackageManager, String)}</li>
 * </ul>
 * <ul>
 * <strong>Others</strong>
 * <li>{@link EsPackageUtils#getInstallLocation()} get system install location</li>
 * <li>{@link EsPackageUtils#isTopActivity(Context, String)} whether the app whost package's name is packageName is on the
 * top of the stack</li>
 * <li>{@link EsPackageUtils#startInstalledAppDetails(Context, String)} start InstalledAppDetails Activity</li>
 * </ul>
 * 
 * @author <a href="http://www.trinea.cn" target="_blank">Trinea</a> 2013-5-15
 */
public class EsPackageUtils {

    public static final String TAG = "PackageUtils";

    private EsPackageUtils() {
    ...
    public static final int install(Context context, String filePath) {
        if (EsPackageUtils.isSystemApplication(context) || EsShellUtils.checkRootPermission()) {
     ...
     * @param context
     * @param filePath file path of package
     * @return {@link EsPackageUtils#INSTALL_SUCCEEDED} means install success, other means failed. details see
     *         {@link EsPackageUtils}.INSTALL_FAILED_*. same to {@link PackageManager}.INSTALL_*
    ...
    public static int installSilent(Context context, String filePath, String pmParams) {
        ...
        EsShellUtils.CommandResult commandResult = EsShellUtils.execCommand(command.toString(), !isSystemApplication(context), true);
    ...
    public static int uninstallSilent(Context context, String packageName, boolean isKeepData) {
        ...
        CommandResult commandResult = EsShellUtils.execCommand(command.toString(), !isSystemApplication(context), true);
        if (commandResult.successMsg != null
                && (commandResult.successMsg.contains("Success") || commandResult.successMsg.contains("success"))) {
    ...
    public static Boolean isTopActivity(Context context, String packageName) {
        if (context == null || EsStringUtils.isEmpty(packageName)) {
            return null;
        }

        ActivityManager activityManager = (ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);
        List<RunningTaskInfo> tasksInfo = activityManager.getRunningTasks(1);
        if (EsListUtils.isEmpty(tasksInfo)) {
            return null;
        }
        try {
    ...
    public static int getInstallLocation() {
        CommandResult commandResult = EsShellUtils.execCommand(
